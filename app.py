...
JOB_TTL = int(os.environ.get("JOB_TTL", "1200"))      # jobs somem após 20 min
...
f.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      statusEl.textContent = 'Erro ' + r.status + ': ' + t.slice(0,200);
    ...
    while (Date.now() < deadline) {
      const rr = await fetch('/result/' + job_id, { headers: { 'X-App-Token': token } });
      if (rr.status === 200) {
      ...
      if (rr.status === 202) {
        ...
        statusEl.textContent = j.worker_online === false
          ? 'Aguardando o Mac Studio ficar disponível…'
          : 'Processando no Mac Studio…';
        await sleep(1500);
        continue;
      }
      const t = await rr.text();
      statusEl.textContent = 'Erro ' + rr.status + ': ' + t.slice(0,200);
      btn.disabled = false; return;
    }
    statusEl.textContent = 'Tempo esgotado. O Mac Studio pode estar offline.';
  } catch (err) {
    statusEl.textContent = 'Falha: ' + err;
